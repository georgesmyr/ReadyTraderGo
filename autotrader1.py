    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.FUTURE:
            ...
            if len(self.etf_best_ask_window) == len(self.etf_best_bid_window) == WINDOW:
                new_bid_price = bid_prices[0] - round(self.etf_spreads_std) * TICK_SIZE_IN_CENTS if bid_prices[0] != 0 else 0
                new_ask_price = ask_prices[0] + round(self.etf_spreads_std) * TICK_SIZE_IN_CENTS if ask_prices[0] != 0 else 0
            else:
            ...
            if self.bid_id == 0 and new_bid_price != 0 and self.etf_position + bid_volume <= POSITION_LIMIT and new_bid_price not in (self.bid_price, 0):
                ...
                self.logger.info(f'MARKET_MAKING:ID:{self.bid_id}:ETF_POS:{self.etf_position}:FUTURE_POS:{self.future_position}')
                self.send_insert_order(self.bid_id, Side.BUY, new_bid_price, bid_volume, Lifespan.GOOD_FOR_DAY)
                self.bids.add(self.bid_id)

            if self.ask_id == 0 and new_ask_price != 0 and self.etf_position - ask_volume >= -POSITION_LIMIT and new_ask_price not in (self.ask_price, 0):
                self.ask_id = next(self.order_ids)
                self.ask_price = new_ask_price
                self.logger.info(f'MARKET_MAKING:ID:{self.ask_id}:ETF_POS:{self.etf_position}:FUTURE_POS:{self.future_position}')
